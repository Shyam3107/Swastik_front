...
export const downloadMissingDocuments =
  (params = {}) =>
  (dispatch) => {
    dispatch({
      type: actionTypes.DOWNLOAD_MISSING_DOCUMENTS_PENDING,
    });

    const options = {
      method: "file",
      url: API.DOWNLOAD_MISSING_DOCUMENTS,
      params: params,
      callback: () => {
        dispatch({
          type: actionTypes.DOWNLOAD_MISSING_DOCUMENTS_SUCCESS,
        });
      },
      errorActionType: actionTypes.DOWNLOAD_MISSING_DOCUMENTS_FAILURE,
      dispatch,
    };
